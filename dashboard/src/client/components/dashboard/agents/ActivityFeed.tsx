...
function ActivityItem({ entry }: { entry: ActivityEntry }) {
  ...
  return (
    <div className="flex gap-2.5 py-2.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
      <span className="text-base shrink-0 mt-0.5">{icon}</span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-xs font-medium text-slate-700 dark:text-slate-200">{emoji} @{entry.agent ?? 'system'}</span>
          {status !== 'created' && (
            <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 ${color}`}>
              → {status}
            </span>
          )}
        </div>
        <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{label}</p>
      </div>
