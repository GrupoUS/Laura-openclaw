...
function Agents() {
  const agentsQuery = trpc.agents.list.useQuery(undefined, { refetchInterval: 30_000 })
  const agents = agentsQuery.data?.data ?? []
  const gwConnected = agentsQuery.data?.connected ?? true
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
        ...
        {agents.length > 0 ? (
          ...
          <div className="text-neutral-500">No agents found.</div>
        ) : null}
      </div>
    </div>
  )
