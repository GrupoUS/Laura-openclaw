...
function ContentPage() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Determine new stage — over.id could be a stage (droppable) or a card id (sortable)
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Ideas → Roteiro → Thumbnail → Gravação → Edição → Publicado
          </p>
        </div>
      ...
      {isError && (
        <div className="bg-red-950/40 border border-red-900 rounded-lg px-4 py-3 text-red-400 text-sm">
          ⚠️ Erro ao carregar o pipeline. Tente recarregar a página.
        </div>
      )}
