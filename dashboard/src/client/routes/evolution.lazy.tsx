...
export const Route = createLazyFileRoute('/evolution')({
  component: Evolution,
})
    ...
    <div className="flex flex-col gap-6">
      ...
      {statsQuery.data && (
        <div className="grid grid-cols-4 gap-4">
          <StatCard label="Total Memories" value={statsQuery.data.memories.total} icon="brain" />
          <StatCard label="Evolution Cycles" value={statsQuery.data.cycles.total} icon="cycle" />
          <StatCard label="Successful" value={statsQuery.data.cycles.successful} icon="check" />
          <StatCard label="Top Capabilities" value={statsQuery.data.topCapabilities.length} icon="diamond" />
        </div>
      )}
  ...
  return (
    <div className="bg-neutral-950 rounded-lg border border-neutral-800 p-5">
      <div className="flex items-center gap-3 mb-2">
        <span className="text-2xl">{icons[icon] ?? icon}</span>
