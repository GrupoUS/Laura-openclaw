...
## Future: TurboRepo CI/CD (Post-Migration)

When the project migrates to TurboRepo monorepo, CI/CD gains workspace-aware optimizations.
...
### Skip Unchanged Apps with `turbo-ignore`

```yaml
- name: Check if API changed
  id: check-api
  run: |
    npx turbo-ignore @repo/api --fallback=HEAD~1 \
      && echo "skip=true" >> $GITHUB_OUTPUT \
      || echo "skip=false" >> $GITHUB_OUTPUT

- name: Build and Deploy API
  if: steps.check-api.outputs.skip != 'true'
...
Remote cache shares build artifacts between CI runs, reducing repeat build times by up to 6x.

→ Full guide: [turborepo-docker.md](turborepo-docker.md)
