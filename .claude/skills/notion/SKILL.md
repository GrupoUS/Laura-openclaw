---
name: notion
description: Use when extracting content from Notion (Pages & Databases) using the Notion MCP server and converting it to high-fidelity Markdown/HTML for website CMS integration.
---
...
- Building a static site with content sourced from Notion.
- Migrating Notion content to another format.

## When NOT to Use

- The user wants to modify Notion content directly (use Notion web app).
- Real-time two-way sync (this skill is extraction only).
- Complex database relationships requiring custom transformation logic.
- Content that requires runtime authentication from end users.
...
3.  For each page:
    ...
    d. `run_command("python3 .claude/skills/notion/scripts/json_to_markdown.py temp.json --output content/hello-world.md --frontmatter '{\"title\": \"Hello World\"}'")`.
    e. Confirm file creation.

## Anti-Patterns

| Pattern                  | Why Avoid                         | Solution                               |
| ------------------------ | --------------------------------- | -------------------------------------- |
| Manual JSON parsing      | Error-prone, misses nested blocks | Use the conversion script              |
| Skipping recursive fetch | Incomplete content                | Always check `has_children`            |
| Hardcoding paths         | Breaks when directory changes     | Use relative paths from workspace root |
| Ignoring rate limits     | API errors, throttling            | Add delays between requests            |

## Validation

Before completing Notion extraction tasks:

- [ ] All blocks with `has_children: true` were recursively fetched
- [ ] JSON was converted using the bundled script, not manual formatting
- [ ] Output files are in the correct directory
- [ ] Frontmatter includes essential metadata (title, date, etc.)
