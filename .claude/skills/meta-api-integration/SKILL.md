...
> **Core Principle:** Centralized config in `meta-config.ts`, signature validation on every webhook, token refresh before expiry.
...
- Generic backend work → use `backend-design` skill
...
This project uses Graph API **v24.0** (env-driven via `META_GRAPH_API_VERSION`).

The version is centralized in `server/_core/meta-config.ts` and reads from the environment:

```typescript
// server/_core/meta-config.ts — single source of truth
export const GRAPH_API_VERSION = process.env.META_GRAPH_API_VERSION ?? "v24.0";
```

The frontend SDK version is set in `client/src/hooks/use-facebook-sdk.ts` (default: `v24.0`).

> ⚠️ When upgrading: update `META_GRAPH_API_VERSION` in `.env`, test all OAuth flows, and verify webhook payloads.
...
| JS SDK Domain Error     | "JSSDK Unknown Host domain"        | Add domain in Meta Dashboard → Use Cases → Facebook Login → Settings → Allowed Domains for the JavaScript SDK |
