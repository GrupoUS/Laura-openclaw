...
> [!CAUTION]
> **`mcp-server-neon` is DEACTIVATED** — the MCP server is no longer running.
> All Neon platform operations MUST use the **`neonctl` CLI** exclusively.
> Never call `mcp_mcp-server-neon_*` tools — they will fail.
...
| Neon platform ops (branches, schema-diff, connections) | Use `neonctl` CLI |
| Neon docs/feature lookup | Fetch `neon.com/docs/<path>.md` |
...
| [CLI-First Neon Operations](references/cli-vs-mcp-standards.md) | When to use CLI vs Drizzle vs API for operations |
| [neonctl Operations](references/neonctl-operations.md) | Full neonctl CLI reference + Neon docs access |
...
## Neon Documentation Access

Any Neon doc page can be fetched as markdown by appending `.md` to the URL:
...
Full index: `https://neon.com/docs/llms.txt`

> Always verify Neon features against official docs — never rely on training data alone.
...
| Use `neonctl` for branch/project ops | Use MCP server for Neon platform ops |
| Fetch Neon docs as `.md` URLs | Guess API behavior from training data |
