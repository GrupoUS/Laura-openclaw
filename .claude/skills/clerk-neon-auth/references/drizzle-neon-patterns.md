...
## 7) neonctl CLI Integration

The `neonctl` CLI complements Drizzle ORM for Neon platform operations:

### Branch Workflows

```bash
# Create feature branch for schema development
...
# Push Drizzle schema to branch
bun run db:push

# Verify schema diff against parent
...
Fetch Neon + Drizzle guides as markdown for agent consumption:
...
> Full neonctl reference: [`./neonctl-operations.md`](./neonctl-operations.md)
