...
try { require('dotenv').config({ path: path.resolve(__dirname, '../../.env'), quiet: true }); } catch (e) {}
...
async function run() {
    ...
    let reportingDirective = process.env.EVOLVE_REPORT_DIRECTIVE || `3.  **📝 REPORT**: Use \`message\` tool to report status.`;
    
    // Phase Task Injection (Dynamic Context)
    let phaseTask = '';
    const phaseTaskPath = path.join(MEMORY_DIR, 'PHASE_TASK.md');
    try {
        if (fs.existsSync(phaseTaskPath)) {
            phaseTask = `\n**🛡️ CURRENT PHASE TASK (HIGHEST PRIORITY)**:\n\`\`\`markdown\n${fs.readFileSync(phaseTaskPath, 'utf8')}\n\`\`\`\n`;
        }
    } catch (e) { console.error('Phase task read error:', e.message); }

    // Legacy Check (Keep for backward compatibility)
    const hasFeishu = process.env.EVOLVE_REPORT_TOOL === 'feishu-card';
    if (hasFeishu && !process.env.EVOLVE_REPORT_DIRECTIVE) {
...
5.  **🔄 RECURSION (Keep Alive)**:
    - **CRITICAL**: This is a continuous process. You MUST trigger the next cycle.
    - **Action**: Run this tool call at the end:
      \`\`\`javascript
      sessions_spawn({
          task: "Run 'node skills/feishu-evolver-wrapper/index.js --loop' and follow the instructions in the output to continue the evolution.",
          agentId: "main",
          cleanup: "delete",
          label: "mad_dog_relay"
      })
