        ...
        {cronsQuery.isLoading ? (
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-neutral-800">
              {(cronsQuery.data as any[])?.length ? (
                (cronsQuery.data as any[]).map((c: any, i) => (
                  <tr key={i} className="hover:bg-neutral-900/50">
                    <td className="px-6 py-4 font-mono">{c.id}</td>
                    <td className="px-6 py-4 font-mono text-indigo-300">{c.schedule}</td>
                    <td className="px-6 py-4 truncate max-w-xs">{c.action || 'Unknown Action'}</td>
